              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    ...
                    {(() => {
                      const taskData = (allTasksData as any)?.data || allTasks || [];
                      const overdueTasks = taskData.filter((task: any) => 
                        task.due_date && 
                        new Date(task.due_date) < new Date() && 
                        task.status?.toLowerCase() !== 'completed'
                      );
                      
                      return overdueTasks.slice(0, 10).map((task: any, index: number) => {
                        ...
                        <TableRow key={task.id || index}>
                          ...
                          <TableCell>{task.owner_name || task.created_by || '-'}</TableCell>
